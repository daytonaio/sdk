...
```python
@overload
def download_file(remote_path: str, timeout: int = 10 * 60) -> bytes
```
...
- `remote_path` _str_ - Path to the file in the Sandbox. Relative paths are resolved based on the user's
  root directory.
- `timeout` _int_ - Timeout for the download operation in seconds. 0 means no timeout. Default is 10 minutes.
...
```python
@overload
def upload_file(file: bytes, remote_path: str, timeout: int = 10 * 60) -> None
```
...
- `remote_path` _str_ - Path to the destination file. Relative paths are resolved based on the user's
  root directory.
- `timeout` _int_ - Timeout for the upload operation in seconds. 0 means no timeout. Default is 10 minutes.
...
def upload_files(files: List[FileUpload], timeout: int = 10 * 60) -> None
```
